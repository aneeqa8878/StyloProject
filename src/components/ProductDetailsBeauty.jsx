...
function ProductDetailsBeauty() {
  ...
  useEffect(() => {
    const data = beauty.filter((selected) => selected.id == params.id);
    setProduct(data[0]);
  }, []);
  return (
    <div>
        ...
        <div className="row">
          ...
          <div className="col-lg-4">
            ...
            <div className="d-flex">
              <button className="btn btn-danger mx-1 my-3 pt-1" onClick={add}>
                +
              </button>
              <h1 className="mx-1 my-3">{count}</h1>
              <button className="btn btn-primary mx-3 my-3  pt-1" onClick={subtract}>
                -
              </button>
